...
def load_filter_config() -> dict:
    ...
    try:
        with open(config_path, "r", encoding="utf-8") as f:
            ...
            return config.get("algorithmic_filter", {
                "enabled": True,
                "title_keywords_any": ["data", "analytics", "bi", "dbt", "etl", "pipeline", "intelligence", "datos", "analista", "ingeniero"],
...
def should_evaluate_job(job: Job) -> Tuple[bool, str]:
    ...
    if not title_matched:
        return False, f"Descarte algorítmico: El título '{job.title}' no contiene palabras clave de datos requeridas."
    ...
    for kw in desc_keywords:
        ...
        if kw_lower not in description:
            return False, f"Descarte algorítmico: La descripción no contiene la palabra clave obligatoria '{kw}'."
